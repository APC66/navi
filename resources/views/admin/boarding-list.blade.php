    ...
    >
      ...
      >
        ...
        <div class="no-print">
          {{-- BOUTON CSV --}}
          <a
            href="{{ admin_url('admin.php?page=navi-boarding-list&sailing_id=' . $selectedSailingId . '&action=download_csv') }}"
            target="_blank"
            class="button button-secondary"
          >
            <span class="dashicons dashicons-media-spreadsheet" style="line-height: 1.3"></span>
            Télécharger CSV
          </a>
          {{-- BOUTON IMPRESSION --}}
          <button onclick="window.print()" class="button button-secondary">🖨️ Imprimer</button>
        </div>
      </div>
      ...
      @else
        ...
        <form id="bulk-action-form" method="POST" action="">
          ...
          <table class="wp-list-table widefat striped table-view-list fixed">
            <thead>
              <tr>
            ...
            <tbody>
              @foreach ($passengers as $pax)
                <tr>
                  ...
                  <td>
                    <strong>{{ $pax['customer_name'] }}</strong>
                    <br />
                    @if (! empty($pax['notes']))
                      <em
                        style="
                  ...
                  <td>
                    <a href="{{ $pax['order_link'] }}" target="_blank">#{{ $pax['order_id'] }}</a>
                  </td>
                  ...
                  <td>
                    @if ($pax['balance_due'] > 0)
                      <span style="color: red; font-weight: bold">
                        +{{ number_format($pax['balance_due'], 2) }}€
                      </span>
...
>
  ...
  >
    ...
    <div style="text-align: right; margin-top: 20px">
      ...
      <button type="button" class="button button-primary" id="modal-confirm-btn">
        Confirmer le report
      </button>
    </div>
  </div>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
      form.addEventListener('submit', function (e) {
        ...
        if (
          document.getElementById('single-action-input').value &&
          document.getElementById('single-action-input').value !== 'reschedule'
        ) {
          return
        }
    ...
    function handleActionLogic(action, contextLabel) {
      if (action === 'reschedule') {
        ...
        if (
          confirm(
            'Êtes-vous sûr de vouloir appliquer l\'action "' +
              action +
              '" à ' +
              contextLabel +
              ' ?',
